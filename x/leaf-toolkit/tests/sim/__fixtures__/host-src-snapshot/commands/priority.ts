...
function priorityDocPath(REPO: string, leaf: ManifestLeaf): string {
  const suffix = leaf.binId ? `.bin-${leaf.binId}` : "";
...
function previewLeaf(REPO: string, l: Leaf): void {
  const out: string[] = [];
  out.push(
    `${ansi("Leaf:", 1)} ${l.path}${l.binIndex ? ` (bin ${l.binIndex}/${l.binTotal})` : ""}`,
  );
  out.push(`${ansi("Files:", 1)} ${l.files.length}, ${ansi("LOC:", 1)} ${l.loc}`);
  ...
  if (auditTxt) {
    ...
    if (riskIdx >= 0) {
      ...
      out.push(...lines.slice(riskIdx, end).map(renderMd));
    }
  }
  process.stdout.write("\n" + out.join("\n").trim() + "\n\n");
}

type PickAns = Level | "skip" | "back";

async function pickPriority(
  REPO: string,
  l: Leaf,
  opts: { allowBack: boolean; defaultValue: Level | null; position: number; total: number },
): Promise<PickAns> {
  previewLeaf(REPO, l);
  const choices: Array<{ name: string; value: PickAns }> = [];
  if (opts.allowBack) choices.push({ name: "← back to previous leaf", value: "back" });
  choices.push({
    name: opts.defaultValue ? `keep (${opts.defaultValue})` : "skip — no priority",
    value: "skip",
  });
  for (const lv of LEVELS) choices.push({ name: lv.label, value: lv.value as Level });
  const ans = await select<PickAns>({
    message: `Priority for ${l.path}${l.binIndex ? ` bin ${l.binIndex}/${l.binTotal}` : ""}  (${opts.position + 1}/${opts.total})`,
    choices,
    loop: false,
    default: opts.defaultValue ?? "skip",
  });
  return ans;
}

// Walk a list of leaves with back-navigation. Returns confirmed assignments
// keyed by absolute LEAF.priority.md path. "skip" leaves the entry untouched
// (the existing on-disk priority, if any, is preserved). "back" rewinds.
async function runAssignments(
  REPO: string,
  leaves: Leaf[],
): Promise<Array<{ leaf: Leaf; level: Level }>> {
  const picks = new Map<string, Level | "skip">();
  let i = 0;
  while (i < leaves.length) {
    const leaf = leaves[i];
    const runPick = picks.get(leaf.notesAbs);
    const def: Level | null =
      runPick && runPick !== "skip" ? runPick : (leaf.current ?? null);
    const ans = await pickPriority(REPO, leaf, {
      allowBack: i > 0,
      defaultValue: def,
      position: i,
      total: leaves.length,
    ...
    picks.set(leaf.notesAbs, ans);
    i++;
  }
  const out: Array<{ leaf: Leaf; level: Level }> = [];
  for (const leaf of leaves) {
    const p = picks.get(leaf.notesAbs);
    if (p && p !== "skip") out.push({ leaf, level: p });
  }
  return out;
}
...
// Tiny markdown→ANSI renderer for audit previews. Strips markup either way;
// adds bold/italic/cyan when TTY. Intentionally inline — keeps the toolkit
// dep-free.
function renderMd(line: string): string {
  const h = line.match(/^(#{1,6})\s+(.*)$/);
  if (h) return ansi(h[2], 1);
  let s = line.replace(/^(\s*)[-*]\s+/, "$1• ");
  s = s.replace(/\*\*([^*]+)\*\*/g, (_, t) => ansi(t, 1));
  s = s.replace(/(^|[^*])\*([^*\n]+)\*/g, (_, lead, t) => `${lead}${ansi(t, 3)}`);
  s = s.replace(/`([^`]+)`/g, (_, t) => ansi(t, 36));
  return s;
}
...
async function distributeMode(REPO: string, leaves: Leaf[]): Promise<void> {
  ...
  const assignments = await runAssignments(REPO, targets);
...
function writePriority(REPO: string, absPath: string, level: Level): void {
  ...
  const leafPath = relative(REPO, absPath).replace(
    /\/LEAF\.priority(\.bin-[A-Za-z0-9]+)?\.md$/,
    "",
  );
  const out =
    `---\ndomain: priority\nleafPath: ${leafPath}\npriority: ${level}\n---\n\n` +
...
export async function priority(argv: string[]): Promise<void> {
  ...
  const assignments = await runAssignments(REPO, picked);
  if (assignments.length === 0) {
    process.stdout.write("\nNo priorities to write.\n");
    return;
  }
