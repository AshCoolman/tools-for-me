...
| Rung | What lives here | What `/spec-next-mini` does to advance |
|---|---|---|
...
| `blocked` | Parked on external dependency | Skipped by `/spec-next-mini` until unblocked. |

One rung per `/spec-next-mini` invocation. No combining `specify + plan` in one go — each rung benefits from a beat of reflection on its predecessor.

## Hand-off boundary

`/spec-next-mini` never implements code. The boundary is at `tasks → implement`: the command writes a one-line nomination to `NEXT.md` (free-form, e.g. `Ship T2 of <spec> — <one-line title>. <spec-path>. (short)`). A runner you configure (see `HANDOFF.md`) reads `NEXT.md`, fetches the task block from the spec, ships it, removes the line from `NEXT.md`, and notes the SHA in the spec's Implement log.
